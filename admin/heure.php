<?php
function totaljourner($date) {
    include ("include/configuration.php");

    $list = array();
    $sql = "SELECT employers.id as id,employers.image as image, employers.nom as nom, poincons.date as temp  FROM poincons INNER JOIN employers ON employers_id = employers.id WHERE poincons.date LIKE '$date%'";
    $result = $conn->query($sql);

    if ($result->rowCount() > 0) {
        echo "<div class='col-6'>";
        echo"<h1 style=\"font-size: 4rem; margin: auto; color: white;\" class=\"text-center\">Log</h1>";
        echo "<ul style='max-height: 350px;' class=\"list-group overflow-auto mx-2 mt-1 to_the_top\">";
        ...
        while ($row = $result->fetch()) {
            ...
            echo "<li onclick=\"myfunction('$id','$nom');\"  class=\"d-inline-block list-group-item list-group-item-action to_the_top\">";
            echo $nom;
            echo " - ".$temp." - ";
            echo "<img src=\"data:image/png;base64,$image\" class=\"d-inline-block ml-5 img-list to_the_top\" alt=\"rfid\">";
            echo "</li>";
        }
    ...
    echo "<div class='col-6'>";
    echo "<ul class=\"ft-10 list-group mx-2 mt-1 to_the_top\">";

    foreach($list as $index=>$number)
    {
//        echo $index ."    ";
        foreach($number as $nom=>$data)
        {
...
//                echo "     -".$heure[1]."<br>";
            ...
            if (count($data) % 2 == 0) {
                ...
                if (count($data) === 4){
                    ...
                    $total2      = strtotime($last2) - strtotime($first2);

                    if ($total2>3600){
                        $depacement = $total2-3600;
                        $hours2     = floor($depacement / 60 / 60);
                        $minutes2    = round(($depacement - ($hours2 * 60 * 60)) / 60);
                        echo "<span class=\"badge badge-warning badge-pill ml-auto\">Pause dépacer de $hours2 h : $minutes2 m</span>";
                    }

                    echo "<span class=\"badge badge-success badge-pill ml-auto mr-1\">ok</span>";

                }
                else if (count($data) !== 4 && count($data)!==2){
                    $all = count($data);
                    echo "<span class=\"badge badge-warning badge-pill ml-auto\">Poinçons à vérifier ($all poincons)</span>";
                }
                else {
                    echo "<span class=\"badge badge-success badge-pill ml-auto mr-1\">ok</span>";
                ...
                echo "<span class=\"badge badge-primary badge-pill\">$hours H $minutes M</span>";

            }
            else{
                $all = count($data);
                echo "<span class=\"badge badge-danger badge-pill ml-auto mr-1\">Erreur</span>";
                echo "<span class=\"badge badge-warning badge-pill\">Poinçons manquant (Présentement $all poinçons)</span>";
            }
            echo "</li>";
